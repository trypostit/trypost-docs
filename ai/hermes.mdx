...
<Steps>
  <Step title="Add the TryPost MCP server">
    ...
    If the add command fails to connect, Hermes may ask **Save config anyway?** and write the entry with `enabled: false`. Open `~/.hermes/config.yaml`, set `enabled: true` (or run `add` again), then continue with `hermes mcp login` below. To give the probe more time next time:
