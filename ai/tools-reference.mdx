...
| **events** | array | Subscribed events: `post.created`, `post.scheduled`, `post.unscheduled`, `post.published`, `post.partially_published`, `post.failed`, `post.deleted`. |
...
| **create-webhook-tool** | Create a webhook. Input: `endpoint` (required public URL), `events` (required, at least one allowed event). Returns the webhook **with** `signing_secret`. Private/local URLs are rejected. Creating does **not** ping the endpoint. |
...
| **send-webhook-test-tool** | POST a signed `webhook.test` ping (`data` is `{}`). Input: `webhook_id`. Returns `{ tested: true }`. Does **not** write a delivery log. Marked `IsOpenWorld`. |
...
| **replay-webhook-log-tool** | Replay a log. Input: `webhook_id` + `log_id`. Queues a **new** signed delivery with a new log id, using the original `data`, even if the webhook is paused or disabled. Returns `{ replayed: true }`. Marked `IsOpenWorld`. |
