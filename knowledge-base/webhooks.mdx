...
There are no events for comments, mentions, social-account disconnects, team changes, or AI Create finishing. Those stay in [notifications](/knowledge-base/notifications). Posts created by an [automation](/knowledge-base/automations/introduction) still fire these events (`created_via` is `automation`). Deleting a [workspace](/knowledge-base/workspaces) removes the webhooks with it and does **not** send `post.deleted` for each post.
...
<AccordionGroup>
  ...
  <Accordion title="Does an automation-created post fire workspace webhooks?">
