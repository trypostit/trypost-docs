...
Watches one Instagram or Facebook account for videos posted outside TryPost and republishes them. Requires **manage repurposes** (Owner / Admin / Member) — Viewers get `Not authorized to manage repurposes.` Other-workspace IDs return `Repurpose not found.` Conceptual guide: [Repurpose](/knowledge-base/repurposes). REST: [`GET /repurposes`](/api-reference/endpoint/list-repurposes).

A new row starts as a **draft**. Only videos published after **activate** are replicated. Videos already published through TryPost are skipped. Destination `content_type` must accept video; `meta` uses the same keys as a regular post.

| Tool | Description |
|------|-------------|
| **list-repurpose-source-formats-tool** | List watchable formats (`reel`, `video`, `story`) with localized labels. No input. Returns `{ source_formats: [...] }` — REST wraps the same rows in `{ data: [...] }`. |
| **list-repurposes-tool** | List workspace repurposes, newest first. Optional `page`. Returns `{ repurposes, total, per_page, current_page, last_page }` (`per_page` is **25**). |
| **get-repurpose-tool** | Read one row by `repurpose_id` (source, destinations, status, last poll error). |
| **create-repurpose-tool** | Create a draft. Input: `source_social_account_id` (required Instagram/Facebook), optional `source_format` (default `reel`), `publish_mode` (`publish` \| `draft`, default `publish`), `destinations[]` (`social_account_id`, `content_type`, optional `meta`). One source + format per workspace. |
| **update-repurpose-tool** | Update `source_social_account_id`, `source_format`, `publish_mode`, and/or `destinations`. Required `repurpose_id`. Changing the source or format resets the watermark. |
| **list-repurpose-items-tool** | Activity for one row. Input: `repurpose_id`, optional `page`. Returns `{ items, total, per_page, current_page, last_page }`. Look here when a video was not replicated. |
| **activate-repurpose-tool** | Start watching. Draft or disabled only. Needs a usable source, one usable destination, and required destination meta. Stamps the watermark to now. |
| **pause-repurpose-tool** | Pause an active row. Watermark kept. Input: `repurpose_id`. |
| **resume-repurpose-tool** | Resume a paused row. User pause keeps the watermark; a system pause (`paused_reason` set) starts from now. |
| **disable-repurpose-tool** | Turn off an active or paused row and clear the watermark. Input: `repurpose_id`. |
| **delete-repurpose-tool** | Delete the row. Polling stops; calendar posts stay. Input: `repurpose_id`. Returns `{ deleted: true }`. |
...
| **create-webhook-tool** | Create a webhook. Input: `endpoint` (required public `http://` or `https://` URL, max 255), `events` (required, at least one allowed event). Always created `enabled` — there is no `status` input. Returns the webhook **with** `signing_secret`. Private/local URLs are rejected. Creating does **not** ping the endpoint. |
...
| **send-webhook-test-tool** | POST a signed `webhook.test` ping (`data` is `{}`). Input: `webhook_id`. **Synchronous** — waits for the HTTP response, then returns `{ tested: true }`. Does **not** write a delivery log and does not change status or failure counters. Runs even if the webhook is paused or disabled. Marked `IsOpenWorld`. |
| **rotate-webhook-secret-tool** | Rotate the signing secret. Input: `webhook_id`. Previous secret stops working immediately — the next attempt, including an in-flight retry, signs with the new secret. Returns the webhook with the new `signing_secret`. Marked `IsDestructive`. |
...
| **replay-webhook-log-tool** | Replay a log. Input: `webhook_id` + `log_id`. Queues a **new** signed delivery with a new log id, using the original `data`, even if the webhook is paused or disabled. `{ replayed: true }` means queued, not delivered. Success resets the failure counter but does **not** re-enable a paused webhook. Marked `IsOpenWorld`. |
