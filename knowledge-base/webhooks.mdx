...
There are no events for comments, mentions, social-account disconnects, or team changes — those stay in [notifications](/knowledge-base/notifications). AI Create has no dedicated `post.ready` event; when the draft is saved you get `post.created` (`created_via` is `web`). [Repurpose](/knowledge-base/repurposes) replicas are ordinary posts (`created_via` is `repurpose`): `post.created`, then `post.scheduled` in publish mode, then the publish outcome. Deleting a [workspace](/knowledge-base/workspaces) removes the webhooks with it and does **not** send `post.deleted` for each post.
