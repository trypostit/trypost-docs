...
Both the file upload endpoint ([`POST /posts/{post}/media`](/api-reference/endpoint/upload-media)) and the URL-fetch endpoint ([`POST /posts/{post}/media/from-url`](/api-reference/endpoint/attach-media-from-url)) accept the same MIME set. WebM is intentionally excluded — every major social platform rejects it, so accepting it would just produce platform-specific publish failures.
