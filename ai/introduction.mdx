...
TryPost's MCP server uses **OAuth 2.1** with Dynamic Client Registration (DCR), PKCE, and the `mcp:use` scope. Modern MCP clients (Claude, Claude Code, Claude Desktop, ChatGPT, Perplexity, Grok, Cursor, Windsurf, VS Code, GitHub Copilot in IDE, Gemini CLI, OpenCode, OpenClaw, Hermes, Codex) detect this automatically from the `.well-known` discovery endpoints — you only point them at the URL and the browser sign-in handles the rest.
...
<Columns cols={3}>
  ...
  <Card title="Windsurf" icon="/images/ai/windsurf.svg" href="/ai/windsurf">
  ...
  <Card title="Hermes" icon="/images/ai/hermes.svg" href="/ai/hermes">
