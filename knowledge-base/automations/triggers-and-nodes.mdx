...
<AccordionGroup>
  ...
  <Accordion title="Webhook" icon="arrow-up-right-from-square">
    ...
    This is a one-off call inside the flow — not a workspace [event webhook](/knowledge-base/webhooks). Use workspace webhooks when you want signed `post.published` / `post.failed` deliveries for every matching post.
  </Accordion>
