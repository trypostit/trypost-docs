...
`SELF_HOSTED=true` is the default. It bypasses everything billing-related so you never have to touch Stripe or Cashier:
...
If you raise the video limit, also bump PHP's `upload_max_filesize` and `post_max_size` and any reverse-proxy body-size cap to match.

## File Storage

TryPost supports local storage and S3-compatible cloud storage.

The default disk is **Cloudflare R2** (`FILESYSTEM_DISK=r2`). For self-hosted installs without object storage configured, switch to `local`.
