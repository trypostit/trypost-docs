...
- **Telegram metrics use a different shape.** The Bot API exposes no likes, comments, shares, or views, so each metric entry carries an extra `kind` field: `"subscribers"` for the channel's member count, and `"reaction"` for each reaction type (where `label` is the emoji). Standard platforms omit `kind` and return likes/comments/shares.
