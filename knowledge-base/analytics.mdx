...
Personal LinkedIn, Bluesky, and Mastodon don't expose account-level analytics — those accounts won't show up in the sidebar. Telegram's tab shows only its subscriber count (the Bot API exposes no view metrics); per-post reactions appear on each published post's metrics instead.
