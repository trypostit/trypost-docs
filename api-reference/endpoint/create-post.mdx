...
<Note>
  **Pinterest posts**: `POST /posts` does not accept per-platform `meta`. After creating, call [`PUT /posts/{post}`](/api-reference/endpoint/update-post) and set `platforms[].meta.board_id` for each Pinterest entry — otherwise the publish job fails with `Pinterest board_id is required`.
</Note>
