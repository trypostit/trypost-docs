...
|------------|-------------------------------------|
| Set `APP_KEY`, `APP_URL`, DB passwords, `REVERB_APP_SECRET` | Wait for Postgres, run migrations |
| `docker compose … up -d` | `passport:keys` if missing, `storage:link` if missing |
| `db:seed` + `UserSeeder` | Cache config/routes/views/events |
...
It does **not** seed the database — next step.

### 4. Seed Passport client and admin user (one-time)
...
<Tip>
  Passport **keys** = entrypoint. Passport **Personal Access Client** = `db:seed`. API keys and MCP need both.
</Tip>
...
2. Create an API key (Settings → API) — proves Passport keys + client exist
3. Schedule a post — Horizon must show as running
