...
<Note>
  **Read accounts by `id`, not by `platform`.** A workspace can hold [several accounts on the same network](/knowledge-base/social-accounts#connection-limits), so the same `platform` value can appear more than once.
</Note>
