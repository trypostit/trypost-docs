...
- Subscription gating is not enforced, so workspaces, social accounts, and members are unlimited per account
...
A workspace can connect as many accounts as you want on the same network — two LinkedIns, two Instagrams, and so on. There is no env flag for this.

Two flavours of the same platform still collapse to **one identity**:

- LinkedIn profile + LinkedIn Page
- Instagram Standalone + Instagram (Facebook Business)

The same identity cannot sit twice. Reconnecting an account that already has a card updates that card instead of adding another.
...
| | PostgreSQL | MySQL |
|---|---|---|
| Covered by the test suite | 16 | 8.4 |
| Expected to work | 14+ | 8+ |
| Shipped by the Compose files | Yes | No — [see below](/self-hosting/docker) |
| Runs TryPost Cloud | Yes | No |
...
Create the database as `utf8mb4` (see [Installation](/self-hosting/installation)) so emoji in posts survive. MySQL 8 already defaults to it; this matters on older servers and on MariaDB.

<Warning>
  **On MySQL you cannot schedule a post past 19 January 2038.**

  TryPost's schema stores dates in MySQL's `TIMESTAMP` columns, and that type ends on 19 January 2038 — a post scheduled beyond it is rejected outright. PostgreSQL's equivalent has no such ceiling, which is why the same schedule saves there.

  This only bites if you schedule more than a decade ahead. If that is part of how you work, choose PostgreSQL.
</Warning>
...
| Value | Behaviour |
|---|---|
| `false` (default) | The X version of a post publishes with its URLs untouched |
| `true` | Every URL in the X version is rewritten: the scheme and `www.` are dropped and every dot of the host becomes `(.)` |

So `https://www.example.com/pricing` publishes as `example(.)com/pricing`, and `https://blog.example.com.br` as `blog(.)example(.)com(.)br`. Every dot has to break — leaving one intact still leaves a domain X can resolve and bill for.
...
- Anything carrying `https://`, `http://` or `www.` is rewritten on sight, whatever it ends in.
- A bare host is rewritten only when its last label is a delegated top-level domain, which is what tells `acme.com` apart from `Node.js`. The list mirrors the full IANA root zone, so `README.md` and `backup.zip` are rewritten too — `.md` and `.zip` are real top-level domains and X links them.
- Email addresses are left alone.
- Dots in the path or query string are left alone.

Publishing and the post previews in the app, the API and MCP all show the same rewritten text, and the editor counts characters against it — what is counted is what X receives.
...
Active [repurposes](/knowledge-base/repurposes) poll Instagram and Facebook for videos published outside TryPost. The scheduler runs `repurpose:poll` every five minutes; each source is checked when it is due.
...
| Variable | Default | Description |
|----------|---------|-------------|
| `REPURPOSE_POLL_INTERVAL_MINUTES` | `15` | Minutes between successful polls of one source |
| `REPURPOSE_BACKOFF_MINUTES` | `60` | Minutes to wait after a rate-limit (or other transient) error from the source |
...
as optional. Sign-in is refused if the provider reports the email address as
unverified, since accounts are matched by email.
...
TRYPOST_USER_AGENT="TryPost.it/1.0 (+https://trypost.it)"
```
