...
- **Media alt text** — image accessibility text lives on `media[].meta.alt_text`, not on `platforms[].meta`. You can set it via `attach-media-from-upload-tool`'s `alt` argument or when sending `media[]` on create/update.