...
| **attach-media-from-url-tool** | Download images or videos from public URLs and attach them to a post. Up to 10 URLs per call, 50 MB per file. Allowed types are intersected with the enabled platforms (e.g. video rejected if no enabled platform supports it). |
