...
<Note>
  The two count as **one network**, so they share the same slot. On TryPost Cloud that means one Instagram account per workspace, by either method. See [how many accounts per network](/knowledge-base/social-accounts#how-many-accounts-per-network).
