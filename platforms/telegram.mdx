...
Each channel or group becomes its own account. Connect as many as you want in the same workspace — see [multiple accounts](/knowledge-base/social-accounts#connection-limits).
...
| `content_type` | Image | Video | Format |
|----------------|-------|-------|--------|
| `telegram_post` | 5 MB | 20 MB | JPEG/PNG/GIF/WebP; video MPEG4 (MP4 or MOV) |

These are Telegram's **URL** limits (TryPost does not use multipart). Official specs: [Sending files](https://core.telegram.org/bots/api#sending-files). Full catalog: [Media](/knowledge-base/media).
