...
If you already have a Personal Access Token, the simpler endpoint is [`POST /posts/{post}/media`](/api-reference/endpoint/upload-media) — direct upload to a specific post, no signed URL. To reuse a file already in the Asset Library, use [`POST /posts/{post}/media/from-asset`](/api-reference/endpoint/attach-existing-asset) instead of uploading again.
