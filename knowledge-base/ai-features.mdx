...
There is no monthly AI quota. Copilot, Generate, Review, and Create stay available on both Cloud plans. You still need an [active subscription](/knowledge-base/plans-and-billing) on Cloud — without one, those actions return `402 Payment Required`.
...
Without those, the AI buttons stay disabled. Pin a model and pick a provider in [AI Providers](/self-hosting/ai). Self-hosted instances skip the Cloud subscription check (`SELF_HOSTED=true` in `.env`).
