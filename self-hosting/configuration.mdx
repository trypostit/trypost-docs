...
TryPost runs on **PostgreSQL** or **MySQL**. Every release runs the full test suite against both, so neither is a second-class path — pick the one you already know how to back up and restore.

| | PostgreSQL | MySQL |
|---|---|---|
| Versions covered by our test suite | 16 | 8.4 |
| Expected to work | 14+ | 8+ |
| Shipped by our Compose files | Yes | No — [see below](/self-hosting/docker) |
| Runs TryPost Cloud | Yes | No |

MariaDB is close enough to MySQL that it may well work, but we do not test it and will not be able to help if it breaks.

Pick one before you have data. Moving an existing install from one engine to the other is not a dump-and-load: the two disagree on types, quoting and JSON layout, so it needs a conversion tool and a careful check afterwards. We do not document or support that path.
...
Create the database as `utf8mb4` so emoji in posts survive:

```sql
CREATE DATABASE trypost CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
```

MySQL 8 already defaults to `utf8mb4`; this matters on older servers and on MariaDB.

<Warning>
  **On MySQL you cannot schedule a post past 19 January 2038.**

  MySQL stores timestamps in a format that ends on that date, so a post scheduled beyond it fails to save. PostgreSQL has no such ceiling.
