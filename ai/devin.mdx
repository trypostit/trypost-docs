...
<Steps>
  ...
  <Step title="Add the TryPost server">
    ...
    Set **Access** to **Personal** so each Devin member signs in to their own TryPost account. **Organization** shares one TryPost login across every member's sessions — only use that if you intend a shared workspace login.
...
<Steps>
  ...
  <Step title="Sign in">
    ...
    Complete the TryPost browser sign-in (**Allow**). To sign in again later:
...
Before CLI v3000.3, MCP servers lived under `mcpServers` inside the main `config.json`. Current versions use the dedicated `mcp_config.json` files above and migrate the old key on startup.

## Devin Desktop (Cascade)

New Devin Desktop tabs use [Devin Local](#devin-cli-and-devin-local). The legacy **Cascade** agent still uses a separate file and `"serverUrl"`.
...
<AccordionGroup>
  ...
  <Accordion title="Enterprise team blocked the server">
    A Devin admin can disable MCP or allowlist only certain servers. If TryPost never connects on a Teams / Enterprise plan, ask an admin to allow a custom HTTP server at `https://app.trypost.it/mcp/trypost` (or your `{APP_URL}/mcp/trypost`).
  </Accordion>
</AccordionGroup>
