...
<ResponseField name="data" type="array">
  <Expandable>
    ...
    <ResponseField name="media" type="array">
      Media items attached to the post. Each item has `id`, `path`, `url`, `type` (`image` | `video` | `document`), `mime_type`, `original_filename`, optional `size` (bytes — included when the item was attached from the Asset Library), and optional `meta` (e.g. `alt_text` for images — this is media meta, not `platforms[].meta`). Empty array if none.
    </ResponseField>
