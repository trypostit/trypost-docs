---
title: "Introduction"
description: "The TryPost API lets you manage social media posts, the Asset Library, signatures, labels, repurposes, webhooks, and more with a simple REST API."
icon: "book-open"
---
...
The endpoints that wrap in `data:` are the paginated lists: [`GET /posts`](/api-reference/endpoint/list-posts), [`GET /assets`](/api-reference/endpoint/list-assets), [`GET /repurposes`](/api-reference/endpoint/list-repurposes), [`GET /repurposes/{id}/items`](/api-reference/endpoint/list-repurpose-items), and [`GET /webhooks/{id}/logs`](/api-reference/endpoint/list-webhook-logs). [`GET /repurpose-source-formats`](/api-reference/endpoint/list-repurpose-source-formats) wraps in `data:` but is not paginated.

## Pagination

These list endpoints paginate. They use Laravel's standard pagination envelope (`data`, `links`, `meta`) and the `page` query parameter:

| Endpoint | Per page | Notes |
|---|---|---|
| [`GET /posts`](/api-reference/endpoint/list-posts) | **15** | Posts only |
| [`GET /assets`](/api-reference/endpoint/list-assets) | **25** | Asset Library. No `per_page` parameter |
| [`GET /webhooks/{id}/logs`](/api-reference/endpoint/list-webhook-logs) | **15** | Delivery logs for one webhook. No `per_page` parameter |
| [`GET /repurposes`](/api-reference/endpoint/list-repurposes) | **25** | Workspace repurposes. No `per_page` parameter |
| [`GET /repurposes/{id}/items`](/api-reference/endpoint/list-repurpose-items) | **25** | Activity for one repurpose. No `per_page` parameter |
...
Use the `page` query parameter to navigate. Other list endpoints (signatures, labels, social accounts, API keys, webhooks) return all items without pagination, as a plain array.

## Quick reference

| Resource | Endpoints |
|---|---|
| Posts | `GET /posts`, `POST /posts`, `GET /posts/{id}`, `PUT /posts/{id}`, `DELETE /posts/{id}`, `POST /posts/{id}/media`, `POST /posts/{id}/media/from-url`, `POST /posts/{id}/media/from-asset`, `GET /posts/{id}/metrics`, `GET /posts/{id}/preview` |
...
| Repurpose | `GET /repurposes`, `POST /repurposes`, `GET /repurposes/{id}`, `PUT /repurposes/{id}`, `GET /repurposes/{id}/items`, `POST /repurposes/{id}/activate`, `POST /repurposes/{id}/pause`, `POST /repurposes/{id}/resume`, `POST /repurposes/{id}/disable`, `DELETE /repurposes/{id}`, `GET /repurpose-source-formats` |
| Webhooks | `GET /webhooks`, `POST /webhooks`, `GET /webhooks/{id}`, `PUT /webhooks/{id}`, `DELETE /webhooks/{id}`, `POST /webhooks/{id}/send-test`, `POST /webhooks/{id}/rotate-secret`, `GET /webhooks/{id}/logs`, `POST /webhooks/{id}/logs/{log}/replay` |
