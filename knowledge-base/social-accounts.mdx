...
## Multiple accounts {#connection-limits}

A workspace can connect as many accounts as you want on the same network — two LinkedIns, three Instagrams, and so on. There is no one-per-network cap.

Two platforms are still two ways into the **same identity** when they point at the same profile or page:

- LinkedIn profile and LinkedIn Page
- Instagram Standalone and Instagram (Facebook Business)

The same identity cannot sit twice under those two flavors. Connecting it the second way updates the existing card instead of creating another.
...
Instagram and Facebook accounts can also be a [repurpose](/knowledge-base/repurposes) source. Switching a source off pauses that flow; reconnecting it resumes the flow automatically. Switching a destination off only skips that account for each new video.
...
| *This workspace already has an account for this network. Disconnect it first.* | A reconnect collided with an identity that is already connected | Reconnect the original card, or disconnect the duplicate first |
...
<AccordionGroup>
  <Accordion title="Can I connect the same account twice?">
    The same identity cannot be connected twice in one workspace. You can connect several accounts on the same network (two Instagrams, for example). To refresh an account you already have, use [Reconnect](#reconnecting-an-account) on its card, which updates that card rather than creating another.
  </Accordion>
