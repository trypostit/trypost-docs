...
Each asset object has: `id`, `original_filename`, `type` (`image` \| `video` \| `document`), `mime_type`, `size` (bytes), `url` (public file URL), `meta` (`width` / `height` / `duration` / `alt_text` when present), `created_at` (`Y-m-d H:i:s`). There is no `path`.
...
| Parameter | Required | Description |
|-----------|----------|-------------|
| **asset_id** | Yes | UUID from `list-assets-tool`. Required. Not a UUID is rejected. Missing IDs, other workspaces, logos, and avatars return `Asset not found.` |
...
| Parameter | Required | Description |
|-----------|----------|-------------|
| **post_id** | Yes | Post UUID in this workspace. Required. Not a UUID is rejected. Post must be `draft` or `scheduled`. Other workspaces return `Post not found.` |
| **asset_id** | Yes | Library UUID from `list-assets-tool` or `get-asset-tool`. Required. Not a UUID is rejected. Missing / other-workspace / non-library IDs return `Asset not found.` |
...
| Returns | Type | Description |
|---------|------|-------------|
| **post** | object | The updated post, same shape as `get-post-tool`. The library item is now in `media[]` (includes `path` and `size`). |
