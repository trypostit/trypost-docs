...
- If you signed up with Google and never set a password, the page shows a "Set password" form instead. Setting one lets you log in either way.
...
If Google login is configured (Cloud has it on; self-hosted needs `GOOGLE_CLIENT_ID` etc.), you can link or unlink your Google account here. Unlinking is blocked if it would leave you with no way to sign in (e.g. you have no password set).
...
You'll be asked to type your password (or "DELETE", if you're a Google-only user) to confirm.
