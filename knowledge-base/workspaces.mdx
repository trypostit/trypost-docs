...
On TryPost Cloud, the plan — not the workspace count — sets the price. **Socials** ($19/month) includes one workspace. **Workspaces** ($99/month) includes as many as you need. Both include unlimited social accounts, unlimited members, and unmetered AI.

If you are on Socials and try to create a second workspace, the upgrade dialog opens. See [Plans & billing](/knowledge-base/plans-and-billing) for full pricing, the $1 first month, and plan-change details. Self-hosted instances skip these checks.
...
<Warning>
  Deleting a workspace permanently removes all posts, [webhooks](/knowledge-base/webhooks) and their logs, disconnects all social accounts, and removes all team members. This cannot be undone. It does not send `post.deleted` for each post — the webhooks are removed with the workspace.
</Warning>
...
<AccordionGroup>
  <Accordion title="How many workspaces can I create?">
    On TryPost Cloud, Socials includes one workspace and Workspaces includes unlimited workspaces. Self-hosted instances have no limit.
  </Accordion>
