...
| **create-post-tool** | Create a draft post. Accepts `content`, `scheduled_at` (ISO 8601), `label_ids[]`, and `platforms[]` — each entry has `social_account_id` + `content_type` and optional `meta`. Same `meta` contract as the REST API ([full reference](/api-reference/endpoint/update-post#per-platform-meta)). Cross-validates that the content_type matches the account's platform. |
| **update-post-tool** | Update a draft: content, scheduled_at, status (draft \| scheduled), labels, per-platform `meta` (merged; send `null` to clear a key), and which platforms are enabled. Cannot edit a published post. |
| **publish-post-tool** | Publish a draft — immediately or scheduled for a future time. Requires at least one enabled platform, and that each enabled platform has the meta it needs to publish (Pinterest `board_id`, TikTok `privacy_level`, Discord `channel_id`) — otherwise it fails with a clear validation error. Marked `IsDestructive` so the AI confirms with you first. |
...
| **list-pinterest-boards-tool** | List boards for a Pinterest account (`account_id`). Returns `{ boards: [{ id, name }], truncated }`. Use a board `id` as `platforms[].meta.board_id` (**required to publish** Pinterest). |
| **list-discord-channels-tool** | List text/announcement channels the bot can post to for a Discord account (`account_id`). Returns `{ channels: [{ id, name }] }`. Use a channel `id` as `platforms[].meta.channel_id` (**required to publish** Discord). |
...
- **Media alt text** — image accessibility text lives on `media[].meta.alt_text`, not on `platforms[].meta`.
