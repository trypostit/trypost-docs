...
Posts created by [Repurpose](/knowledge-base/repurposes) also land in the calendar (`created_via` is `repurpose`).
...
Workspace [webhooks](/knowledge-base/webhooks) fire on those status changes (`post.scheduled`, `post.published`, `post.partially_published`, `post.failed`). `publishing` itself does not send an event.
...
From the post list (or any post's "..." menu) pick **Duplicate** to copy a post into a new draft. The duplicate gets a fresh ID and `draft` status; the original is untouched. The copy preserves content, media, labels, and platform entries — adjust the schedule and platforms before publishing. Subscribed webhooks receive `post.created` for the copy.
...
You can delete any post at any time. Deleting removes the post and all its platform entries. Subscribed webhooks receive `post.deleted` with only `{ id, workspace_id }`.
...
<AccordionGroup>
  ...
  <Accordion title="Why does my link read example(.)com on X?">
    X charges much more to deliver a post that carries a link and shows it to fewer people, so the X version of a post is published with its links written non-clickable. Only X is affected, and your draft is untouched. See [links in posts](/platforms/x-twitter#links-in-posts).
  </Accordion>
</AccordionGroup>
