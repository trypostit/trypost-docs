...
Max file size: **1 GB**. Files larger than ~1 MB are sent in chunks via `Content-Range` for reliability on flaky connections.

## Importing from URL

The post composer also accepts public URLs — TryPost downloads the file server-side, validates the MIME type and size against the enabled platforms on the post, and stores the result. From the API, this is [`POST /posts/{post}/media`](/api-reference/endpoint/attach-media).

URL imports additionally accept `video/quicktime` (MOV) and `video/webm` on top of the standard formats above.
...
There's no public API for the asset library yet. The closest equivalent is [`POST /posts/{post}/media`](/api-reference/endpoint/attach-media), which downloads from a URL and attaches the result directly to a specific post in one shot.
